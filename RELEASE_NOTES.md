...
- **Dual-channel release pipeline.** `.github/workflows/release.yml` now drives two release channels off two branches:
  ...
  - **`develop` → canary.** Every push produces a pre-release tagged `canary-v<base>-canary.<run-number>` with all four platform bundles. The suffix is the monotonic GitHub Actions run counter — MSI bundles require numeric-only pre-release identifiers (max 65535), so the commit SHA can't go in the version itself; it's surfaced in the release title + body instead for traceability. A rolling `canary-latest` release holds only the updater manifest so installed canary apps poll a fixed URL while the per-commit releases preserve history.
