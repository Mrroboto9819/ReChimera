...
- **Dual-channel release pipeline.** `.github/workflows/release.yml` now drives two release channels off two branches:
  ...
  - **`develop` → canary.** Every push produces a pre-release tagged `canary-v<base>-<run-number>` with all four platform bundles. The suffix is the monotonic GitHub Actions run counter — MSI bundles require the SemVer pre-release portion to be a single numeric value (max 65535), no words or dots, so neither the commit SHA nor the word "canary" can live in the version string itself; the SHA is surfaced in the release title + body, and "canary" identity is conveyed by the product name, bundle identifier, icon, and brand color. A rolling `canary-latest` release holds only the updater manifest so installed canary apps poll a fixed URL while the per-commit releases preserve history.
...
- **`bun run version:canary` for local canary builds.** New flag in `scripts/sync-version.mjs --canary [--suffix=<n>]` mirrors what CI does — patches `tauri.conf.json` (version / productName / identifier / updater endpoint), flips the default `brandColor` in `store.ts`, and regenerates the bundled icon set from `icon_canary.png`. Useful for testing a canary bundle locally before pushing to `develop`. Suffix auto-detects from `git rev-list --count HEAD` (a monotonic integer); the `--suffix=<n>` escape hatch accepts any numeric value ≤ 65535.
